 ...
 *
 * @package   oauth-generator
 * @version   1.0.1
 * @author    Amjad khan <amjad.khan.contact@gmail.com>
 * @copyright 2021 Amjad khan <amjad.khan.contact@gmail.com>
 * @license   http://opensource.org/licenses/GPL-3.0 GNU General Public License 3.0
 * @link      https://github.com/amjadkhanroot/oauth-helper
 */
...
class OAuthV1
{
    ...
    public function validate(
        $authorization, $httpmethod, $url, $multipart = false
    )
    {
        // parse parameters
        $apiparams = $this->_parseApiParams($this->_params);
        ...
        if ($httpmethod === 'GET' || !$multipart) {
            $correct_authorization = $this->_sign(
                $httpmethod, $url, $this->_params, $timestamp, $nonce
            );
        } else {
            $correct_authorization = $this->_sign(
     ...
     */
    protected function _sign($httpmethod, $method, $timestamp, $nonce)
    {
     ...
     * @return string Authorization HTTP header
     */
    public function sign($httpmethod, $method, $timestamp, $nonce)
    {
        if (self::$_oauth_consumer_key === null) {
            throw new \Exception('To generate a signature, the consumer key must be set.');
        }
